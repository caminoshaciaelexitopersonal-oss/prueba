...
from database.db_manager import agregar_comprobante_y_movimientos, obtener_cuentas_puc, obtener_comprobantes, obtener_movimientos_por_comprobante
from logic.contabilidad_logic import validar_partida_doble
...
class RegistroContableViewContent(ft.UserControl):
    ...
    def agregar_fila_movimiento(self, e):
        ...
        # Crear la fila
        new_row = ft.Row(
            [cuenta_dropdown, desc_detalle_field, debito_field, credito_field, delete_button],
            alignment=ft.MainAxisAlignment.START,
    ...
    def guardar_comprobante(self, e):
        ...
        if not validar_partida_doble(movimientos_data):
             mostrar_snackbar(self.page, "Error de Partida Doble: La suma de débitos no es igual a la suma de créditos.")
             return

        # Intentar guardar en la base de datos
        success, comprobante_id = agregar_comprobante_y_movimientos(
            fecha=fecha,
            tipo=tipo,
    ...
    def cargar_comprobantes_recientes(self, limit=20):
        ...
        comprobantes_db = obtener_comprobantes(limit=limit)
    ...
    def mostrar_detalle_comprobante(self, e):
        ...
        movimientos = obtener_movimientos_por_comprobante(comprobante_id)
...
def route_cliente(page: ft.Page):
    from cliente.cliente_view import ClienteViewRoute
    return ClienteViewRoute(page)