# views/inventario_view.py
import flet as ft
import asyncio
from inventario.agents.corps.general_inventario import InventarioGeneral
from inventario import inventario_logic
from utils.helpers import format_currency

# --- Clases para la Interfaz de Chat ---
class Message:
    def __init__(self, user_name: str, text: str, message_type: str):
        self.user_name = user_name
        self.text = text
        self.message_type = message_type

class ChatMessage(ft.Row):
    def __init__(self, message: Message):
        super().__init__(vertical_alignment=ft.CrossAxisAlignment.START)
        self.controls = [
            ft.CircleAvatar(
                content=ft.Text(message.user_name[:1].capitalize()),
                color=ft.Colors.WHITE,
                bgcolor=ft.Colors.BLUE_GREY_600 if message.message_type == "bot_message" else ft.Colors.GREEN_600
            ),
            ft.Column(
                [
                    ft.Text(message.user_name, weight=ft.FontWeight.BOLD),
                    ft.Text(message.text, selectable=True, width=700),
                ],
                tight=True,
                spacing=5,
            ),
        ]

# --- Contenido Principal de la Vista ---
class InventarioViewContent(ft.UserControl):
    def __init__(self, page: ft.Page):
        super().__init__(expand=True)
        self.page = page
        self.general_inventario = InventarioGeneral()

        # --- Controles de la UI ---
        self.chat_list = ft.ListView(expand=True, spacing=10, auto_scroll=True)
        self.new_message = ft.TextField(
            hint_text="Escribe tu orden de inventario aquí...",
            autofocus=True,
            shift_enter=True,
            min_lines=1,
            max_lines=5,
            filled=True,
            expand=True,
            on_submit=self.send_message_click,
        )
        self.progress_ring = ft.ProgressRing(visible=False)

        self.tabla_productos = ft.DataTable(
            columns=[
                ft.DataColumn(ft.Text("ID")),
                ft.DataColumn(ft.Text("SKU")),
                ft.DataColumn(ft.Text("Nombre")),
                ft.DataColumn(ft.Text("Stock"), numeric=True),
                ft.DataColumn(ft.Text("Costo Promedio"), numeric=True),
            ],
            rows=[],
            expand=True,
        )

    def did_mount(self):
        self.add_message(Message("General Inventario", "Hola, soy el General de Inventario. Puedes darme órdenes como 'crear un producto' o 'registrar una compra'.", "bot_message"))
        self.cargar_productos()

    def build(self):
        return ft.Row(
            [
                # Columna de la tabla de productos
                ft.Column(
                    [
                        ft.Text("Productos en Inventario", size=18, weight=ft.FontWeight.BOLD),
                        ft.IconButton(icon=ft.icons.REFRESH, on_click=lambda e: self.cargar_productos(), tooltip="Recargar Tabla"),
                        ft.Container(content=self.tabla_productos, expand=True, border=ft.border.all(1, ft.Colors.OUTLINE), border_radius=5),
                    ],
                    expand=2, # Ocupa 2/3 del espacio
                ),
                ft.VerticalDivider(width=1),
                # Columna del Agente de IA
                ft.Column(
                    [
                        ft.Text("Asistente de IA", size=18, weight=ft.FontWeight.BOLD),
                        self.chat_list,
                        ft.Row([self.progress_ring], alignment=ft.MainAxisAlignment.CENTER),
                        ft.Row(
                            [
                                self.new_message,
                                ft.IconButton(
                                    icon=ft.icons.SEND_ROUNDED,
                                    tooltip="Enviar Orden",
                                    on_click=self.send_message_click,
                                ),
                            ],
                        ),
                    ],
                    expand=1, # Ocupa 1/3 del espacio
                ),
            ],
            expand=True,
            vertical_alignment=ft.CrossAxisAlignment.STRETCH
        )

    def add_message(self, message: Message):
        self.chat_list.controls.append(ChatMessage(message))
        self.update()

    async def send_message_click(self, e):
        user_message_text = self.new_message.value
        if not user_message_text: return

        self.new_message.value = ""
        self.add_message(Message("Tú", user_message_text, "user_message"))
        self.progress_ring.visible = True
        self.update()

        try:
            bot_response_text = await asyncio.to_thread(
                self.general_inventario.process_command, user_message_text
            )
            self.add_message(Message("General Inventario", bot_response_text, "bot_message"))
            # Después de una operación exitosa, recargar la tabla para ver los cambios
            self.cargar_productos()
        except Exception as ex:
            self.add_message(Message("Error", f"Ocurrió un error: {ex}", "bot_message"))
        finally:
            self.progress_ring.visible = False
            self.update()

    def cargar_productos(self):
        self.tabla_productos.rows.clear()
        productos = inventario_logic.obtener_productos()
        if isinstance(productos, str): # Si la herramienta devuelve un string JSON
            try:
                productos = json.loads(productos)
            except json.JSONDecodeError:
                self.add_message(Message("Error", "No se pudo cargar la lista de productos.", "bot_message"))
                return

        for p in productos:
            self.tabla_productos.rows.append(
                ft.DataRow(cells=[
                    ft.DataCell(ft.Text(p.get('id', ''))),
                    ft.DataCell(ft.Text(p.get('sku', ''))),
                    ft.DataCell(ft.Text(p.get('nombre', ''))),
                    ft.DataCell(ft.Text(str(p.get('cantidad_disponible', 0)))),
                    ft.DataCell(ft.Text(format_currency(p.get('costo_unitario_promedio', 0)))),
                ])
            )
        self.update()

# --- Función para crear la vista completa (usada por el router) ---
def InventarioView(page: ft.Page):
    return ft.View(
        "/inventario",
        appbar=ft.AppBar(
            title=ft.Text("Gestión de Inventario"),
            bgcolor=ft.Colors.SURFACE_VARIANT,
            leading=ft.IconButton(ft.icons.ARROW_BACK, on_click=lambda _: page.go("/dashboard"), tooltip="Volver al Dashboard")
        ),
        controls=[
            InventarioViewContent(page)
        ],
        padding=15
    )
